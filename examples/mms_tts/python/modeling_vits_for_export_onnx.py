...
import os
randn_like_latents_path = os.environ.get('randn_like_latents_path')
...
def _unconstrained_rational_quadratic_spline(
    ...
    Returns:
        ...
        log_abs_det (`torch.FloatTensor` of shape `(batch_size, channels, seq_len)`:
            ...
            limits applied.
    """
    # Do not use GreaterOrEqual/LessOrEqual to avoid accuracy loss!!
...
def _rational_quadratic_spline(
    ...
    cumheights = torch.zeros_like(heights)
    for i in range(heights.size(-1)):
        if i == 0:
            cumheights[..., i] = heights[..., i]
        else:
            cumheights[..., i] = cumheights[..., i - 1] + heights[..., i]
    ...
    # Do not use GreaterOrEqual/LessOrEqual to avoid accuracy loss!!
    # bin_idx = torch.sum(inputs.unsqueeze(-1) >= bin_locations, dim=-1) - 1
    bin_idx = torch.sum(-inputs.unsqueeze(-1) < -bin_locations, dim=-1) - 1
