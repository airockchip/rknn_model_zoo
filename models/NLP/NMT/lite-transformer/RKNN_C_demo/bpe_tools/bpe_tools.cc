...
int Bpe_Tools::prepare_token_data(const char* token_dict_path, DICT_TYPE dict_type){
    FILE *fp_dict;
    ...
    fclose(fp_dict);
    printf("prepare token_data done\n");

    return 0;
}


int Bpe_Tools::prepare_bpe_data(const char* bpe_dict_path, DICT_TYPE dict_type){
    FILE *fp_dict;
...
int Bpe_Tools::bpe_encode(const char* word, char bpe_word_list[WORD_LEN_LIMIT][WORD_LEN_LIMIT]){
    ...
    for (int i = 0; i < num_char; i++)
    {
        ...
        num_char += 1;
#elif (BPE_VERSION == 0)
        sprintf(sub_word[num_char-1], "%s%s", sub_word[num_char-1], SPLIT_SYM);         // add split symbol
#endif
