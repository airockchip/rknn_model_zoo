...
{
    printf("  index=%d, name=%s, n_dims=%d, dims=[%d, %d, %d, %d], n_elems=%d, size=%d, fmt=%d, type=%d, qnt_type=%d, "
           "zp=%d, scale=%f\n",
           attr->index, attr->name, attr->n_dims, attr->dims[0], attr->dims[1], attr->dims[2], attr->dims[3],
           attr->n_elems, attr->size, attr->fmt, attr->type,
...
int main(int argc, char **argv)
{
    ...
    unsigned char *model_data = load_model(encoder_model_path, &model_data_size);
    ret1 = rknn_init(&ctx1, (void *)model_data, model_data_size, 0);
    if (ret1 < 0)
    {
    ...
    model_data = load_model(decoder_model_path, &model_data_size);
    ret2 = rknn_init(&ctx2, (void *)model_data, model_data_size, 0);
    if (ret2 < 0)
    {
        printf("rknn_init fail! ret=%d\n", ret2);
        return -1;
