...
int NC1HWC2_i8_to_NCHW_fp32(const int8_t* src, float* dst, int* dims, int channel, int h, int w, int zp, float scale)
{
  int batch  = dims[0];
  int C1     = dims[1];
  int C2     = dims[4];
...
int NC1HWC2_fp16_to_NCHW_fp32(const float16* src, float* dst, int* dims, int channel, int h, int w, int zp, float scale)
{
  ...
  for (int i = 0; i < batch; i++) {
    ...
    for (int c = 0; c < channel; ++c) {
      int            plane  = c / C2;
      const float16* src_c  = plane * hw_src * C2 + src;
      int            offset = c % C2;
      for (int cur_h = 0; cur_h < h; ++cur_h)
        for (int cur_w = 0; cur_w < w; ++cur_w) {
          int cur_hw                          = cur_h * w + cur_w;
          dst[c * hw_dst + cur_h * w + cur_w] = fp16_to_fp32(src_c[C2 * cur_hw + offset]); // float16-->float
        }
    }
  }
  ...
  for (uint32_t i = 0; i < io_num.n_output; i++) {
    if (output_attrs[i].fmt == RKNN_TENSOR_NC1HWC2) {
      ...
      if (orig_output_attrs[i].type == RKNN_TENSOR_INT8) {
        NC1HWC2_i8_to_NCHW_fp32((int8_t*)output_mems[i]->virt_addr, (float*)output_mems_nchw[i],
      ...
      if (output_attrs[i].type == RKNN_TENSOR_INT8) {
        ...
        float16* src = (float16*)output_mems[i]->virt_addr;
        float*   dst = output_mems_nchw[i];
        for (int index = 0; index < output_attrs[i].n_elems; index++) {
          dst[index] = fp16_to_fp32(src[index]);
        }
      } else {
        printf("dtype: %s cannot convert!", get_type_string(orig_output_attrs[i].type));
      }
