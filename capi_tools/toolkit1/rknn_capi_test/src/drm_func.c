...
void* drm_buf_alloc(drm_context* drm_ctx, int drm_fd, int TexWidth, int TexHeight, int bpp, int* fd,
                    unsigned int* handle, size_t* actual_size)
{
  int ret;
  if (drm_ctx == NULL) {
  ...
  // buffer\n",alloc_arg.width,alloc_arg.height,alloc_arg.bpp,alloc_arg.size); printf("out handle=
  // %d\n",alloc_arg.handle);
